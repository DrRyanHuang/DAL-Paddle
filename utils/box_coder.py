...
class BoxCoder(object):
    ...
    def encode(self, ex_rois, gt_rois):
        ...
        ex_widths = paddle.clip(ex_widths, min=1)
        ex_heights = paddle.clip(ex_heights, min=1)
    ...
    def decode(self, boxes, deltas, mode='xywht'):
        ...
        widths = paddle.clip(widths, min=1)
        heights = paddle.clip(heights, min=1)
        ...
        pred_w = widths if 'w' not in mode else paddle.exp(dw) * widths
        pred_h = heights if 'h' not in mode else paddle.exp(dh) * heights
        pred_t = thetas if 't' not in mode else paddle.atan(paddle.tan(thetas / 180.0 * np.pi) + dt) / np.pi * 180.0
